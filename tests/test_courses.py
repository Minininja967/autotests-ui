...
def test_empty_courses_list(chromium_page_with_state: Page):
    ...
    courses_header = chromium_page_with_state.get_by_test_id('courses-list-empty-view-title-text')
    expect(courses_header).to_be_visible()
    expect(courses_header).to_have_text('There is no results')

    courses_text = chromium_page_with_state.get_by_test_id('courses-list-empty-view-description-text')
