...
class CreateCoursePage(BasePage):
    def __init__(self, page: Page):
        ...
        self.exercises_empty_view_description = (
            page.get_by_test_id('create-course-exercises-empty-view-description-text')
        )
    ...
    def check_visible_exercise_form(self, index: int, title: str, description:str):
        exercise_subtitle_text = self.page.get_by_test_id(
        ...
        expect(exercise_subtitle_text).to_have_text(f'#{index + 1} Exercise')

        expect(exercise_title_input.locator('label')).to_be_visible()
        expect(exercise_title_input.locator('label')).to_have_text('Title')
        expect(exercise_title_input.locator('input')).to_be_visible()
        expect(exercise_title_input.locator('input')).to_have_value(title)

        expect(exercise_description_input.locator('label')).to_be_visible()
        expect(exercise_description_input.locator('label')).to_have_text('Description')
        expect(exercise_description_input.locator('input')).to_be_visible()
        expect(exercise_description_input.locator('input')).to_have_value(description)

    def fill_visible_exercise_form(self, index: int, title: str, description: str):
        exercise_title_input = self.page.get_by_test_id(
            f'create-course-exercise-form-title-{index}-input'
        ).locator('input')
